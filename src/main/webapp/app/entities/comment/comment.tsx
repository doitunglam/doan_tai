            ...
            <tbody>
              {commentList.map((comment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {comment.createdDate ? <TextFormat type="date" value={comment.createdDate} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>{comment.vote}</td>
                  <td>{comment.services ? <Link to={`services/${comment.services.id}`}>{comment.services.id}</Link> : ''}</td>
